...
import { GraduationCap, ChevronRight, User, FileText, ShieldCheck, HelpCircle } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  const handleLogout = () => {
    Alert.alert('Sign Out', 'Are you sure you want to sign out?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          await logout();
          // Assuming an Auth navigator is setup separately, but for now we reset or navigate.
  ...
  const menuItems = [
    { icon: <User size={18} color="#4F46E5" />, label: 'Edit Profile', color: '#4F46E5' },
    { icon: <FileText size={18} color="#0891B2" />, label: 'My Resume', color: '#0891B2' },
    { icon: <ShieldCheck size={18} color="#7C3AED" />, label: 'Change Password', color: '#7C3AED' },
    { icon: <HelpCircle size={18} color="#64748B" />, label: 'Help & Support', color: '#64748B' },
  ];
