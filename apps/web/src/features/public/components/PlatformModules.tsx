      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          >
            ...
            <p className="text-lg text-slate-500 leading-relaxed">
              A unified ecosystem providing specialized interfaces tailored perfectly for every stakeholder in the placement process.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {modules.map((mod, index) => (
            ...
            >
              <div className={`p-8 lg:p-10 h-full rounded-2xl bg-white border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 overflow-hidden relative group`}>
                {/* Subtle corner decoration */}
                <div className={`absolute -right-12 -top-12 w-32 h-32 rounded-full ${mod.color} transition-transform duration-500 group-hover:scale-150 ease-out`} />
                
                <div className="relative z-10 flex flex-col h-full">
                  <div className={`shrink-0 mb-6 inline-flex p-4 rounded-xl shadow-sm border bg-white ${mod.borderColor}`}>
                    {mod.icon}
                  </div>
                  <div>
