        ...
        <div className="divide-y divide-slate-100">
          {isLoading ? (
            ...
            <div className="p-16 text-center text-slate-500 flex flex-col items-center justify-center">
              ...
              <p className="text-slate-500 max-w-sm">You're all caught up! Check back later for updates on drives and interviews.</p>
            </div>
          ) : (
            filteredNotifications.map((notification: any) => (
              ...
              >
                <div 
                  ...
                  notification.type === 'placement_drive' ? 'bg-blue-100 text-blue-600' :
                  notification.type === 'interview' ? 'bg-purple-100 text-purple-600' :
                  'bg-slate-100 text-slate-600'
                }`}>
                  {notification.type === 'placement_drive' ? <CalendarDays className="w-5 h-5" /> :
                   notification.type === 'interview' ? <Clock className="w-5 h-5" /> :
                ...
                >
                  <div className="flex justify-between items-start">
                    <h4 className={`text-base ${!notification.isRead ? 'font-bold text-slate-900' : 'font-semibold text-slate-700'}`}>
                      {notification.title}
                    </h4>
