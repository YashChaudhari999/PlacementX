...
import { Calendar, Clock, MapPin, Briefcase, FileText, Info, Download, ArrowRight, ExternalLink } from 'lucide-react';
...
export const CalendarEventDetails: React.FC<CalendarEventDetailsProps> = ({ event, isOpen, onClose }) => {
        ...
        <div className="px-6 pt-5 pb-3 bg-muted/20 border-b border-border/40">
          <div className="mb-4 pr-8">
            <div className="flex items-center justify-between mb-3.5">
              <Badge variant="outline" className="bg-background font-semibold tracking-wide uppercase text-[10px] text-muted-foreground border-border/80 shadow-sm">
                {props.type || 'Event'}
              </Badge>
              {props.status && (
                <Badge variant={props.status === 'Open' || props.status === 'Scheduled' ? 'success' : 'secondary'} className="shadow-sm">
                  {props.status}
                </Badge>
              )}
            </div>
            ...
            {(props.description || props.instructions) && (
              <div className="flex items-start gap-3 pt-5 border-t border-border/60">
                ...
                <div className="space-y-4 flex-1">
                  {props.description && (
                    <div>
                      <span className="font-semibold block text-foreground mb-1.5 uppercase text-xs tracking-wider text-muted-foreground">Description</span>
                      <p className="whitespace-pre-wrap text-foreground/90 leading-relaxed bg-muted/10 p-4 rounded-xl border border-border/40">{props.description}</p>
                    </div>
                  )}
                  {props.instructions && (
                    <div>
                      <span className="font-semibold block text-foreground mb-1.5 uppercase text-xs tracking-wider text-muted-foreground">Instructions</span>
                      <p className="whitespace-pre-wrap text-foreground/90 leading-relaxed bg-primary/5 p-4 rounded-xl border border-primary/20 text-primary-900">{props.instructions}</p>
                    </div>
                  )}
                </div>
              </div>
