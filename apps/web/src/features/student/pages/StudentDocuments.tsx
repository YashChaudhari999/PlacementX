...
export default function StudentDocuments() {
 ...
 {/* Header Section */}
 <div className="flex justify-between items-center bg-card/70 p-8 rounded-3xl backdrop-blur-xl border border-border/80 shadow-sm relative overflow-hidden">
 ...
 Manage your academic records, primary resume, and official placement offer letters in
 one secure place.
 </p>
 </div>
 </div>
 ...
 {/* Primary Resume */}
 <Card className="p-6 border-border/60 shadow-lg shadow-slate-100 bg-card/90 backdrop-blur-xl hover:border-indigo-200 transition-all duration-300">
 <h3 className="font-bold text-foreground flex items-center gap-2 mb-4">
 <File01Icon className="w-5 h-5 text-indigo-600"/> Primary Resume
 </h3>

 <div
 className={`p-5 border rounded-xl flex flex-col items-center justify-center text-center transition-all ${data.academicDocuments.find((d: any) => d.documentType === 'RESUME') ? 'border-emerald-100 bg-success-muted/50' : 'border-dashed border-border bg-muted/50'}`}
 >
 {data.academicDocuments.find((d: any) => d.documentType === 'RESUME') ? (
 <>
 <div className="w-12 h-12 bg-emerald-100 text-success rounded-full flex items-center justify-center mb-3">
 <TickDouble02Icon className="w-6 h-6"/>
 </div>
 <p className="font-medium text-emerald-800 mb-4">Resume Uploaded & Active</p>
 <Button
 variant="outline"
 className="w-full bg-card border-emerald-200 hover:bg-success-muted hover:text-emerald-700"
 onClick={() =>
 window.open(
 data.academicDocuments.find((d: any) => d.documentType === 'RESUME')
 ?.signedUrl || '',
 ...
 <p className="font-medium text-foreground mb-1">No Resume Found</p>
 <p className="text-xs text-muted-foreground mb-4">
 You must upload your resume in your profile before applying to drives.
 </p>
 ...
 Upload clear PDF scans of your marksheets. These are required for your profile
 completion and are verified by the placement cell. Max 15MB each.
 </p>
 ...
 const uploadedDoc = data.academicDocuments?.find(
 ...
 <div
 key={docDef.type}
 className={`p-4 border rounded-xl transition-all ${uploadedDoc ? 'border-indigo-100 bg-indigo-50/30' : 'border-border bg-muted'}`}
 >
 ...
 <TickDouble02Icon className="w-5 h-5 text-success shrink-0"/>
 )}
 </div>
 ...
 variant="outline"
 size="sm"
 className="flex-1 bg-card hover:bg-muted text-xs h-8"
 ...
 type="file"
 accept="application/pdf"
 aria-label={`Upload ${docDef.title}`}
 ...
 />
 <div
 className={`w-full py-2.5 px-3 border border-dashed rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-colors ${uploading ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-card border-border text-muted-foreground group-hover:border-indigo-400 group-hover:bg-indigo-50/50 group-hover:text-indigo-600'}`}
 >
 {uploading ? (
 <>
 ...
 <Card className="p-8 h-full border-border/60 shadow-lg shadow-slate-100 bg-card/90 backdrop-blur-xl">
 <div className="flex items-center justify-between mb-8">
 <div>
 <h3 className="font-bold text-xl text-foreground flex items-center gap-2">
 <Building02Icon className="w-6 h-6 text-success"/> Official Offer Letters
 </h3>
 ...
 <div className="w-20 h-20 bg-card rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-border">
 <Building02Icon className="w-10 h-10 text-slate-300"/>
 </div>
 <h4 className="text-lg font-semibold text-foreground mb-1">No Offers Yet</h4>
 <p className="text-sm text-muted-foreground max-w-sm mx-auto">
 When you are selected in a placement drive and the company issues an official
 letter, it will securely appear here.
 </p>
 </div>
 ) : (
 ...
 <div
 key={offer.id}
 className="border border-border rounded-2xl p-5 flex flex-col justify-between hover:border-emerald-300 hover:shadow-md transition-all bg-card relative overflow-hidden group"
 >
 ...
 <TickDouble02Icon className="w-5 h-5 text-success"/>
 </div>

 <h4 className="font-bold text-lg text-foreground leading-tight mb-1">
 {offer.company}
 </h4>
 <p className="font-medium text-indigo-600 text-sm mb-3">{offer.role}</p>

 <div className="flex items-center gap-1.5 text-xs text-muted-foreground bg-muted px-2.5 py-1.5 rounded-md inline-flex">
