...
export default function AdminProfileVerifications() {
  ...
  const handleAction = (id: string, action: 'APPROVE' | 'REJECT') => {
    verifyMutation.mutate({ id, action, reason: action === 'REJECT' ? reason : undefined }, {
      onSuccess: () => {
  ...
  if (isLoading) return <div className="p-8 text-center text-slate-500 font-medium">Loading pending verifications...</div>;
                ...
                filteredProfiles.map((profile: any) => (
                  <tr key={profile.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{profile.firstName} {profile.lastName}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500">{selectedProfile.firstName} {selectedProfile.lastName}</p>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Branch</div>
                  <div className="text-slate-900">{selectedProfile.branch || 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Current Semester</div>
                  <div className="text-slate-900">{selectedProfile.currentSemester || 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">CGPA</div>
                  <div className="text-slate-900">{selectedProfile.cgpa || 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Passing Year</div>
                  <div className="text-slate-900">{selectedProfile.passingYear || 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Active Backlogs</div>
                  <div className="text-slate-900">{selectedProfile.activeBacklogs || '0'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Backlogs</div>
                  <div className="text-slate-900">{selectedProfile.totalBacklogs || '0'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">10th %</div>
                  <div className="text-slate-900">{selectedProfile.tenthPercentage ? `${selectedProfile.tenthPercentage}%` : 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">12th %</div>
                  <div className="text-slate-900">{selectedProfile.twelfthPercentage ? `${selectedProfile.twelfthPercentage}%` : 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Diploma %</div>
                  <div className="text-slate-900">{selectedProfile.diplomaPercentage ? `${selectedProfile.diplomaPercentage}%` : 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Year Gap</div>
                  <div className="text-slate-900">{selectedProfile.yearGap || '0'}</div>
                </div>
                <div className="col-span-2">
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Resume Link</div>
                  <div className="text-slate-900 break-all">{selectedProfile.resumeUrl ? <a href={selectedProfile.resumeUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">{selectedProfile.resumeUrl}</a> : 'N/A'}</div>
                </div>
                <div className="col-span-2">
              ...
              <div className="pt-4 border-t border-slate-100">
                <label className="block text-sm font-bold text-slate-700 mb-2">Rejection Reason (Optional)</label>
                <textarea 
                  value={reason}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <Button variant="ghost" onClick={() => { setSelectedProfile(null); setReason(''); }}>Cancel</Button>
              <Button 
