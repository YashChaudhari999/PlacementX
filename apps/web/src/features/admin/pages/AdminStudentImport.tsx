...
export default function AdminStudentImport() {
  ...
  const handleImport = async () => {
    ...
    if (validationErrors.length > 0) {
       if (!confirm(`You have ${validationErrors.length} rows with errors. Do you want to proceed importing only the valid rows?`)) {
          return;
       }
    }
    ...
    try {
      ...
      if (result.failed === 0 && result.skipped === 0) {
        ...
        toast.success(`Import finished. Success: ${result.imported}, Failed: ${result.failed}, Skipped: ${result.skipped}`);
      }
    } catch (error: any) {
  ...
  const downloadReport = () => {
    if (importResult) {
      StudentImportService.downloadReport(importResult.reportData, `Student_Import_Report_${new Date().getTime()}.xlsx`);
    }
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold mb-8 text-gray-900 dark:text-white">Admin Student Import</h1>
      ...
      {!file && !importResult && (
        <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 text-center">
          ...
          <h2 className="text-xl font-semibold mb-2 text-gray-800 dark:text-gray-100">Upload CSV File</h2>
          <p className="text-gray-500 mb-6">Drag and drop your student records CSV, or click to browse.</p>
          <input
            type="file"
            accept=".csv"
            className="hidden"
          ...
          <div className="mt-8 text-sm text-gray-500 mb-6">
            Ensure your CSV matches the exact template format. <a href="#" onClick={handleDownloadTemplate} className="text-blue-500 hover:underline">Download Template</a>
          </div>
          ...
          <div className="mt-4 text-sm text-gray-500 text-left bg-gray-50 dark:bg-gray-900/50 p-5 rounded-lg border border-gray-200 dark:border-gray-700 inline-block w-full max-w-3xl">
            ...
            <div className="flex flex-wrap gap-2 text-xs font-mono">
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Roll Number*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">First Name*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Last Name*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Email*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Phone*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Gender*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">Branch*</span>
              <span className="px-2 py-1 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 text-gray-400 border-dashed" title="Default is Phone if empty">Password</span>
            </div>
            <p className="mt-3 text-xs text-gray-400">* Required fields. Password will default to Phone if left empty.</p>
          </div>
        </div>
      )}
      ...
      {file && !importResult && !isImporting && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={reset} className="text-gray-500 hover:text-gray-700 cursor-pointer">Cancel</button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-center justify-between">
               <div>
                 <div className="text-sm text-green-600 dark:text-green-400 font-medium">Valid Records</div>
                 <div className="text-3xl font-bold text-green-700 dark:text-green-300">{parsedData.length}</div>
               </div>
            ...
            <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-center justify-between">
               <div>
                 <div className="text-sm text-red-600 dark:text-red-400 font-medium">Validation Errors</div>
                 <div className="text-3xl font-bold text-red-700 dark:text-red-300">{validationErrors.length}</div>
               </div>
                ...
                <div className="max-h-60 overflow-y-auto bg-red-50/50 dark:bg-red-900/10 border border-red-100 rounded-lg">
                  <table className="min-w-full text-sm">
                     <thead className="bg-red-100 dark:bg-red-900/50">
                       <tr>
                         <th className="py-2 px-4 text-left font-medium text-red-800 dark:text-red-300 w-24">Row</th>
                         <th className="py-2 px-4 text-left font-medium text-red-800 dark:text-red-300">Errors</th>
                       </tr>
                     </thead>
                     <tbody className="divide-y divide-red-200 dark:divide-red-800/30">
                       {validationErrors.slice(0, 50).map((err, i) => (
                         <tr key={i}>
                           <td className="py-2 px-4 font-mono text-gray-600 dark:text-gray-400">{err.row}</td>
                           <td className="py-2 px-4 text-red-600 dark:text-red-400">
                              {Object.entries(err.errors).map(([field, msg]) => (
                                <div key={field}><span className="font-semibold">{field}:</span> {msg}</div>
                              ))}
                           </td>
                         </tr>
                       ))}
      ...
      {isImporting && (
        <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 text-center">
           <h2 className="text-xl font-semibold mb-6 text-gray-800 dark:text-gray-100">Importing Students...</h2>
      ...
      {importResult && !isImporting && (
         <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
           <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Import Completed</h2>
           ...
           {importResult.errors.length > 0 && (
             <div className="mb-8 text-sm text-red-500 bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
                ...
                {importResult.errors.length > 10 && <p className="mt-2 italic">And {importResult.errors.length - 10} more...</p>}
             </div>
           )}
