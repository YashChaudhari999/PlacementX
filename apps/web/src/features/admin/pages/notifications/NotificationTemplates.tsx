...
export default function NotificationTemplates() {
 ...
 <div className="col-span-full p-16 text-center text-muted-foreground flex flex-col items-center justify-center">
 <Layout01Icon className="w-10 h-10 mb-3 text-slate-300"/>
 <h4 className="text-base font-semibold text-foreground">No templates found</h4>
 ...
 <Card
 key={template.id}
 className="border-border shadow-sm flex flex-col transition-shadow hover:shadow-md"
 >
 ...
 <button
 className={`p-1.5 rounded-full ${template.isFavorite ? 'text-warning bg-warning-muted' : 'text-slate-300 hover:text-warning hover:bg-warning-muted'}`}
 >
 <StarIcon
 className="w-4 h-4"
 ...
 <div className="p-5 flex-1 bg-muted/50">
 <div className="text-sm font-semibold text-foreground mb-1">{template.title}</div>
 <div className="text-sm text-muted-foreground line-clamp-3">{template.message}</div>
 ...
 <span
 key={i}
 className="inline-block px-1.5 py-0.5 bg-indigo-50 text-indigo-700 text-[10px] font-mono rounded"
 >
 {v}
 </span>
 ))}
 ...
 <button
 className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive-muted rounded"
 title="Delete"
 >
