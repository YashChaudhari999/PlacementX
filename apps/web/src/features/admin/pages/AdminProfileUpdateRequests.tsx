...
export default function AdminProfileUpdateRequests() {
  ...
  const handleAction = (id: string, action: 'APPROVE' | 'REJECT') => {
    reviewMutation.mutate({ id, action, reason: action === 'REJECT' ? adminComment : undefined }, {
      onSuccess: () => {
  ...
  if (isLoading) return <div className="p-8 text-center text-slate-500 font-medium">Loading update requests...</div>;
                ...
                filteredRequests.map((req: any) => (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{req.student?.firstName} {req.student?.lastName}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500">{selectedRequest.student?.firstName} {selectedRequest.student?.lastName}</p>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 mb-3 uppercase tracking-wider">Requested Changes</h3>
              ...
              <div className="pt-4 border-t border-slate-100">
                <label className="block text-sm font-bold text-slate-700 mb-2">Admin Comment (Optional / Required if Rejecting)</label>
                <textarea 
                  value={adminComment}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <Button variant="ghost" onClick={() => { setSelectedRequest(null); setAdminComment(''); }}>Cancel</Button>
              <Button 
