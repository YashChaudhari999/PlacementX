    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 border-slate-200 bg-white">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Drives</div>
        ...
        <Card className="p-4 border-slate-200 bg-white">
          ...
          <div className="text-2xl font-black text-slate-900">{summary.totalApplications.toLocaleString()}</div>
          <div className="text-xs text-slate-500 font-medium mt-1">Across all drives</div>
        </Card>

        <Card className="p-4 border-slate-200 bg-white">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Avg Apps/Drive</div>
          <div className="text-2xl font-black text-slate-900">{summary.avgApplicationsPerDrive.toFixed(0)}</div>
          <div className="text-xs text-slate-500 font-medium mt-1">Student engagement</div>
        </Card>

        <Card className="p-4 border-slate-200 bg-white">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Offers</div>
                ...
                <tr key={drive.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 min-w-[200px]">
                    <div className="flex gap-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                      <div className="flex items-center gap-1"><Users className="w-3 h-3 text-blue-500" /> {drive.applied}</div>
                      <div className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-emerald-500" /> {drive.offered}</div>
                    </div>
                    <div className="flex h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      {drive.totalApplications > 0 && (
                        <>
                          <div className="bg-emerald-500" style={{ width: `${(drive.offered / drive.totalApplications) * 100}%` }} />
                          <div className="bg-indigo-500" style={{ width: `${(drive.interviewed / drive.totalApplications) * 100}%` }} />
                          <div className="bg-blue-500" style={{ width: `${(drive.shortlisted / drive.totalApplications) * 100}%` }} />
                        </>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className={`font-black ${drive.offerConversion > 10 ? 'text-emerald-600' : drive.offerConversion > 0 ? 'text-slate-900' : 'text-slate-400'}`}>
