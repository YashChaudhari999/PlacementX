...
import { Lightbulb, AlertTriangle, ArrowUpRight, TrendingUp, TrendingDown, Minus, Info } from 'lucide-react';
    ...
    <Card className="p-6 border-slate-200">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {summary.critical > 0 && <span className="px-2 py-1 bg-rose-100 text-rose-700 text-xs font-bold rounded-lg">{summary.critical} Critical</span>}
          {summary.positive > 0 && <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-lg">{summary.positive} Positive</span>}
          <span className="px-2 py-1 bg-slate-100 text-slate-600 text-xs font-bold rounded-lg">{summary.total} Total</span>
        </div>
      </div>
      ...
      <div className="grid gap-3">
        {insights.map((insight: PlacementInsight) => {
          ...
          return (
            <div key={insight.id} className={`p-4 rounded-xl border ${style} flex flex-col md:flex-row gap-4`}>
              ...
              <div className="md:w-48 shrink-0 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center border-t md:border-t-0 md:border-l border-current/10 pt-3 md:pt-0 md:pl-4">
                <div className="text-left md:text-right">
                  <div className="text-[10px] font-bold uppercase tracking-wider opacity-60 mb-1">{insight.metric}</div>
                  ...
                  {insight.gap !== null && (
                    <div className="text-xs font-bold flex items-center md:justify-end gap-1 mt-0.5 opacity-80">
                      {insight.gap > 0 ? <TrendingUp className="w-3 h-3" /> : insight.gap < 0 ? <TrendingDown className="w-3 h-3" /> : <Minus className="w-3 h-3" />}
                      {Math.abs(insight.gap).toFixed(1)} {insight.gap > 0 ? 'above' : 'below'} benchmark
                    </div>
                  )}
                </div>
              </div>
