...
function KPICard({
 ...
 return (
 <Card
 className={`p-5 border-border hover:shadow-md transition-shadow group relative ${
 size === 'large' ? 'col-span-1 md:col-span-2 lg:col-span-1' : ''
 }`}
 >
 {tooltip && (
 <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
 <div className="text-[10px] text-muted-foreground bg-muted rounded-md px-2 py-1 max-w-[200px]">
 {tooltip}
 </div>
 </div>
 )}
 ...
 <p
 className={`font-black text-foreground mt-1 tabular-nums ${
 ...
 {isPositive && <ArrowUp01Icon className="w-3.5 h-3.5 text-success"/>}
 {isNegative && <ArrowDown01Icon className="w-3.5 h-3.5 text-rose-500"/>}
 {isNeutral && <MinusSignIcon className="w-3.5 h-3.5 text-muted-foreground"/>}
 <span
 className={`text-xs font-bold tabular-nums ${
 isPositive ? 'text-success' : isNegative ? 'text-rose-600' : 'text-muted-foreground'
 }`}
 >
 {isPositive ? '+' : ''}
 {change.toFixed(1)}
