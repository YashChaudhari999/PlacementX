...
const generateChartData = (forecast: ForecastResponse) => {
  const currentYearStr = forecast.targetYear; // e.g., "2026/2027"
  const baseYear = parseInt(currentYearStr.split('/')[0]); // 2026
  
  const baseRate = forecast.projectedPlacementRate ? forecast.projectedPlacementRate - 5 : 75;
  const rateDelta = forecast.projectedPlacementRate ? (forecast.projectedPlacementRate - baseRate) / 3 : 2;

  const data = [
    { year: `${baseYear - 3}/${baseYear - 2}`, placementRate: baseRate - (rateDelta * 3), isHistorical: true },
    { year: `${baseYear - 2}/${baseYear - 1}`, placementRate: baseRate - (rateDelta * 2), isHistorical: true },
    { year: `${baseYear - 1}/${baseYear}`, placementRate: baseRate, isHistorical: true },
    { 
      year: forecast.targetYear, 
      placementRate: forecast.projectedPlacementRate || baseRate, 
      isForecast: true,
      range: forecast.confidenceInterval ? [forecast.confidenceInterval[0], forecast.confidenceInterval[1]] : null
    }
  ];
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="p-4 bg-indigo-50/50 rounded-xl border border-indigo-100">
              <div className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Projected Rate</div>
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
              <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Trend</div>
          ...
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} domain={['dataMin - 10', 'dataMax + 10']} />
                <Tooltip 
                ...
                <ReferenceLine 
                  ...
                  stroke="#94a3b8" 
                  strokeDasharray="3 3" 
                  label={{ position: 'top', value: 'Forecast Start', fill: '#94a3b8', fontSize: 10 }} 
                />
