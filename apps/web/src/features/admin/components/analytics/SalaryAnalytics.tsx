...
export default function SalaryAnalytics({ data }: { data: SalaryResponse }) {
 ...
 <div className="p-2 bg-emerald-100 rounded-xl text-success">
 <Money01Icon className="w-5 h-5"/>
 </div>
 <div>
 <h2 className="text-lg font-bold text-foreground">Package Distribution</h2>
 <p className="text-xs text-muted-foreground">Salary ranges across all placed students</p>
 </div>
 </div>
 ...
 ₹{current.averagePackage.toFixed(2)}L
 </div>
 </div>
 <div className="p-4 bg-muted rounded-xl border border-border">
 <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">
 ...
 <div className="text-xl font-black text-foreground">
 ₹{current.medianPackage.toFixed(2)}L
 </div>
 </div>
 <div className="p-4 bg-muted rounded-xl border border-border">
 <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">
 ...
 <div className="text-xl font-black text-foreground">₹{current.p25.toFixed(2)}L</div>
 </div>
 <div className="p-4 bg-muted rounded-xl border border-border">
 <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">
 Upper Quartile (P75)
 </div>
 <div className="text-xl font-black text-foreground">₹{current.p75.toFixed(2)}L</div>
 </div>
 </div>
 ...
 <ReferenceLine
 x={
 current.distribution.find((d) => {
 const parts = d.label.replace('+', '-999').split('-').map(Number);
 const min = parts[0] ?? 0;
 const max = parts[1] ?? 999;
 return current.averagePackage >= min && current.averagePackage <= max;
 })?.label
 }
 stroke="#3b82f6"
 strokeDasharray="3 3"
 label={{ position: 'top', value: 'Avg', fill: '#3b82f6', fontSize: 10 }}
 />
