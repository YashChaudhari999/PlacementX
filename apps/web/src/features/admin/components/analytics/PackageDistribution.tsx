  ...
  return (
    <Card className="p-6 border-slate-200">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">Histogram of salary ranges for placed students.</p>
      </div>
        ...
        {labels.map((label, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center justify-end group h-full relative">
              ...
              <span className="text-xs font-bold text-slate-500 mt-3 text-center whitespace-nowrap truncate w-full px-1">{label}</span>
            </div>
          );
        })}
      </div>
