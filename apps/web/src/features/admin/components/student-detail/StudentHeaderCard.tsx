...
export function StudentHeaderCard({
 ...
 return (
 <Card className="bg-card p-6 shadow-sm border border-border rounded-xl">
 ...
 <h1 className="text-2xl font-bold text-foreground">{importedData.fullName}</h1>
 <Badge
 ...
 <Badge className="bg-info-muted text-blue-700 border-blue-200">Profile Complete</Badge>
 )}
 </div>
 <div className="mt-2 flex flex-wrap items-center gap-y-2 gap-x-4 text-sm text-muted-foreground">
 <div className="flex items-center gap-1.5">
 <Mail01Icon className="w-4 h-4 text-muted-foreground"/>
 <span>{importedData.email}</span>
 </div>
 <div className="flex items-center gap-1.5">
 <BookOpen01Icon className="w-4 h-4 text-muted-foreground"/>
 <span>{importedData.studentId}</span>
 </div>
 {profileData?.phone && (
 <div className="flex items-center gap-1.5">
 <CallIcon className="w-4 h-4 text-muted-foreground"/>
 <span>{profileData.phone}</span>
 </div>
 )}
 {profileData?.address && (
 <div className="flex items-center gap-1.5">
 <MapsLocation01Icon className="w-4 h-4 text-muted-foreground"/>
 ...
 <p className="text-sm font-medium text-muted-foreground mb-1">Backlogs</p>
 <p
 className={`text-2xl font-bold ${importedData.activeBacklogs > 0 ? 'text-destructive' : 'text-foreground'}`}
 >
