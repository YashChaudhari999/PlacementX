import { X, Calendar as CalendarIcon, Clock, MapPin, Building, Info, ExternalLink, Edit2, Trash2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        >
          ...
          <button onClick={onClose} className="relative z-10 p-1 text-white/80 hover:text-white hover:bg-white/20 rounded-full transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {extendedProps?.description && (
            <div className="flex gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-slate-600 mt-0.5 whitespace-pre-wrap leading-relaxed">{extendedProps.description}</div>
              </div>
            </div>
          )}
        </div>
